...
- **Engine reference guides** — Three engine-specific docs ship with the `game-design` module: `engine-phaser.md` (Phaser — 2D game framework with Arcade/Matter physics, tilemaps, scenes), `engine-pixijs.md` (PixiJS — fast 2D renderer, bring-your-own game loop/physics), `engine-threejs.md` (Three.js — 3D rendering with GLTF loading, lighting, shadows, Rapier physics integration). Each covers project setup, core API patterns, project structure conventions, and tuning parameter patterns.
