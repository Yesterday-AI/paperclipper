...
function PhaseIndicator({ phase, questionCount }: { phase: InterviewPhase; questionCount: number }) {
  const phases = [
      ...
      {phases.map((p, i) => {
        ...
        return (
          <div key={p.key} className="flex items-center gap-1">
            {i > 0 && (
              <div className={cn("w-6 h-px", isDone ? "bg-foreground" : "bg-border")} />
            )}
            <div
              className={cn(
...
export function StepAiWizard() {
  ...
  const buildCatalog = () => {
    return [
      "## Available Presets",
      ...state.presets.map((p) => `- **${p.name}**: ${p.description} (modules: ${p.modules?.join(", ")})`),
      "",
      "## Available Modules",
      ...state.modules.map((m) => `- **${m.name}**: ${m.description}`),
      "",
      "## Available Extra Roles",
      ...state.roles.filter((r) => !r._base).map((r) => `- **${r.name}** (${r.title}): ${r.description}`),
      "",
    ...
    return (
      <div className="space-y-6">
        <div className="space-y-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Describe your company or idea. Our AI will interview you and assemble the perfect agent team.
            </p>
          </div>
        </div>
        ...
        <div className="flex justify-end gap-2">
          <Button
            variant="outline"
            onClick={() => dispatch({ type: "GO_TO", step: "onboarding" })}
          >
            Back
          </Button>
          <Button
  ...
  if (phase === "configuring") {
    return (
      <div className="space-y-6">
        <PhaseIndicator phase={phase} questionCount={questionCount} />
        <ConfiguringAnimation
          ready={configReady}
          onDone={applyConfigToState}
        />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <PhaseIndicator phase={phase} questionCount={questionCount} />
        <Button variant="ghost" size="sm" onClick={resetInterview} className="text-muted-foreground">
